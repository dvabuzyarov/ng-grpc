...
export class Proto {
    ...
    getDependencyPackageName(proto: Proto) {
        const name = proto.pb_package ?
            proto.pb_package.replace(/\.([a-z])/g, v => v.toUpperCase())
                .replace(/\./g, "") :
            "noPackage";

        // we always need index to avoid accidental collisions, see type.pb.ts
        const index = String(this.resolved.dependencies.indexOf(proto)).padStart(3, "0");
